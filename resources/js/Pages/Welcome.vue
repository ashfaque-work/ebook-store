<script setup>
import GuestLayout from '@/Layouts/GuestLayout.vue';
import { Head, Link } from '@inertiajs/vue3';

defineProps({
    books: Array,
});
</script>

<template>

    <Head title="Welcome" />

    <GuestLayout>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200">Our Latest eBooks</h2>

                <div v-if="books && books.length > 0"
                    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <Link v-for="book in books" :key="book.id" :href="`/books/${book.slug}`" class="group">
                    <div
                        class="bg-gray-50 dark:bg-gray-700 rounded-lg overflow-hidden transform hover:scale-105 transition-transform duration-300">
                        <img :src="book.cover_image_path" alt="Book Cover" class="w-full h-64 object-cover">
                        <div class="p-4">
                            <h3 class="font-bold text-lg text-gray-900 dark:text-white">{{ book.title }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">by {{ book.author.name }}</p>
                            <p class="mt-2 font-semibold text-blue-600 dark:text-blue-400">${{ book.price }}</p>
                        </div>
                    </div>
                    </Link>
                </div>
                <div v-else>
                    <p class="text-gray-500 dark:text-gray-400">No books are available at the moment. Please check back
                        later!</p>
                </div>
            </div>
        </div>
    </GuestLayout>
</template>
