...
const lightThemeClasses = computed(() =>
    props.active
...
const darkThemeClasses = computed(() =>
    props.active
        ? 'block w-full pl-4 pr-4 py-2 text-left text-sm font-medium text-white bg-gray-900 focus:outline-hidden focus:bg-gray-700 transition duration-150 ease-in-out'
        : 'block w-full pl-4 pr-4 py-2 text-left text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-hidden focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out'
);

const classes = computed(() => {
    return props.theme === 'dark' ? darkThemeClasses.value : lightThemeClasses.value;
});
